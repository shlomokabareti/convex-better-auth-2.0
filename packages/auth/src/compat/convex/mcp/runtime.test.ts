...
describe("mcp oauth runtime helpers", () => {
  ...
  it("rejects management-client scope escalation before session or code creation", async () => {
    ...
    const response = await handleMcpOAuthAuthorizeRequest({
      request: new Request(
        "https://crm.test/oauth/crm-mcp/authorize?response_type=code&client_id=management-client&redirect_uri=https%3A%2F%2Fmanagement.convex.nyc%2Fcallback&scope=crm%3Agrowth%3Awrite&code_challenge=challenge&code_challenge_method=S256"
      ),
      defaultAudience: "https://api.convex.nyc",
      defaultResourceId: "crm-mcp",
      resolveClient: async () => ({
        clientId: "management-client",
        name: "Convex Management",
        redirectUris: ["https://management.convex.nyc/callback"],
  ...
  it("stores an allowed client-scope subset after user authorization", async () => {
    ...
    const response = await handleMcpOAuthAuthorizeRequest({
      request: new Request(
        "https://crm.test/oauth/crm-mcp/authorize?response_type=code&client_id=management-client&redirect_uri=https%3A%2F%2Fmanagement.convex.nyc%2Fcallback&scope=crm%3Aorganization%3Aread&code_challenge=challenge&code_challenge_method=S256",
        {
          headers: {
    ...
    const response = await handleMcpOAuthClientRegistrationRequest({
      request: new Request("https://crm.test/oauth/crm-mcp/register", {
        ...
        body: JSON.stringify({
          ...
          redirect_uris: ["https://app.convex.nyc/oauth/callback"],
          scope: "crm:organization:read",
        }),
      }),
  ...
  it("creates route-safe oauth handlers", async () => {
    ...
    const authorizeResponse = await handlers.handleAuthorizeRequest(
      new Request(
        "https://crm.test/oauth/crm-mcp/authorize?response_type=code&client_id=test&redirect_uri=https%3A%2F%2Fapp.convex.nyc%2Fcallback&scope=crm%3Aorganization%3Aread&code_challenge=abc&code_challenge_method=S256"
      )
    );

...
describe("mcp protocol helpers", () => {
  it("builds stable default paths", () => {
    assert.deepEqual(buildMcpOAuthPaths(config), {
      ...
      mcpPath: "/mcp",
      authorizationServerMetadataPath: "/.well-known/oauth-authorization-server/crm-mcp",
      protectedResourceMetadataPath: "/.well-known/oauth-protected-resource/crm-mcp",
  ...
  it("builds authorization metadata", () => {
    assert.deepEqual(buildAuthorizationServerMetadata("https://crm.test", config), {
  ...
  it("builds protected resource metadata", () => {
    assert.deepEqual(buildProtectedResourceMetadata("https://crm.test", config), {
      resource: "crm:mcp",
  ...
  it("normalizes protocol config", () => {
    assert.deepEqual(
      createMcpOAuthProtocolConfig({
        ...
        audience: " crm-mcp ",
        oauthBasePath: "/oauth/",
        scopesSupported: ["crm:organization:read", "crm:organization:read", "  "],
      }),
      {
        resourceSlug: "crm-mcp",
        resourceId: "crm:mcp",
  ...
  it("resolves request origin and empty jwks", () => {
    assert.equal(
      resolveRequestOrigin(new Request("https://crm.test/oauth/crm-mcp/authorize")),
      "https://crm.test",
    );
    assert.equal(buildMcpOAuthIssuer("https://crm.test", config), "https://crm.test/oauth/crm-mcp");

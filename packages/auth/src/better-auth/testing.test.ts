...
describe("better-auth testing helpers", () => {
...
describe("better-auth preflight command", () => {
  it("fails auth preflight command when trusted origins do not cover the web app", async () => {
    ...
    const lines: string[] = [];
    const exitCode = await runBetterAuthPreflightCommand({
      repoRoot,
      env: {},

...
const handlePagination = async (
  ...
  }) => Promise<SetOptional<PaginationResult<any>, "page"> & { count?: number }>,
  ...
  const onResult = (result: SetOptional<PaginationResult<any>, "page"> & { count?: number }) => {
    state.cursor =
...
const selectDocFields = (doc: DocWithFlexibleId & Record<string, unknown>, select?: string[]) => {
  if (!select?.length) {
    return doc;
  }
      ...
      return {
        ...
        findMany: async (data): Promise<any[]> => {
          ...
          if (data.where?.some((w) => w.connector === "OR")) {
            ...
            if (data.sortBy) {
              docs = sortBy(docs, [prop(data.sortBy.field), data.sortBy.direction]);
            }
        ...
        update: async (data): Promise<any> => {
          ...
          const onUpdateHandle =
            config.authFunctions?.onUpdate && config.triggers?.[data.model]?.onUpdate
              ? ((await createFunctionHandle(
        ...
        consumeOne: async (data): Promise<any> => {
          ...
          const onDeleteHandle =
            config.authFunctions?.onDelete && config.triggers?.[data.model]?.onDelete
              ? ((await createFunctionHandle(
        ...
        deleteMany: async (data) => {
          ...
          const onDeleteHandle =
            config.authFunctions?.onDelete && config.triggers?.[data.model as TableNames]?.onDelete
              ? ((await createFunctionHandle(
                  config.authFunctions.onDelete,
                )) as FunctionHandle<"mutation">)
              : undefined;

...
>(
  ...
  const getHeaders = async (ctx: GenericCtx<DataModel>) => {
    ...
    return new Headers({
      ...
      ...(session?.ipAddress ? { "x-forwarded-for": session.ipAddress as string } : {}),
    });
  };
  ...
  return {
     ...
     * @deprecated in 0.9
     */
    setUserId: async (ctx: GenericMutationCtx<DataModel>, authId: string, userId: string) => {
      ...
      const cors = corsRouter(http, {
        allowedOrigins: async (request) => {
          const resolvedTrustedOrigins =
            trustedOriginsOption ?? (await staticAuth.$context).options.trustedOrigins ?? [];
        ...
        allowedHeaders: ["Content-Type", "Better-Auth-Cookie", "Authorization"].concat(
    ...
    registerRoutesLazy: <T extends CreateAuth<DataModel>>(
      ...
      const getRegistrationAuth = (): ReturnType<T> => {
        registrationAuth = registrationAuth ?? (createAuth({} as any) as ReturnType<T>);

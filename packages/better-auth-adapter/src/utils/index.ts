...
export type TrustedOriginsOption =
  ...
  | ((request?: Request) => (string | null | undefined)[] | Promise<(string | null | undefined)[]>);
...
export const getToken = async (siteUrl: string, headers: Headers, opts?: GetTokenOptions) => {
  headers.set("host", new URL(siteUrl).host);
  const fetchToken = async () => {
    const basePath = opts?.basePath
      ? (opts.basePath.startsWith("/") ? opts.basePath : `/${opts.basePath}`).replace(/\/+$/, "")
      : "/api/auth";
    const { data } = await betterFetch<{ token: string }>(`${basePath}/convex/token`, {
      baseURL: siteUrl,
      headers,
    });

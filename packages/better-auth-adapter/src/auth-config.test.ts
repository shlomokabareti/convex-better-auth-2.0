...
describe("getAuthConfigProvider", () => {
  ...
  it.each(["https://deployment.convex.site?region=us", "https://deployment.convex.site#fragment"])(
    "rejects URL components in the Convex site URL %s",
    (siteUrl) => {
      process.env.CONVEX_SITE_URL = siteUrl;
  ...
  it("accepts an HTTP URL for a local Convex deployment", () => {
    ...
    expect(getAuthConfigProvider().jwks).toBe("http://127.0.0.1:3211/api/auth/convex/jwks");
  });
});
